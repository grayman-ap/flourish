...
import { getTenantVoucherCount } from "@/app/netvend/lib/db";
...
const MobileViewLayout = () => {
  ...
  return (
    <div className="fixed flex flex-col justify-center text-center w-full sm:w-4/6 md:w-3/6 lg:w-2/6 rounded-none shadow-lg gap-2 h-full bg-white">
      <MobileAppHeader header={tenant ? `${tenant.name} Networks` : 'Starlink Network'}/>
      <SubscriptionPlan />
    </div>
  );
}
...
const SubscriptionPlanTabs = () => {
  ...
  const { data: tab, isLoading, error } = useQuery({
    queryKey: ["subPlans"],
    queryFn: () => ["hourly", "daily", "weekly", "monthly"],
    gcTime: 1000 * 60 * 60,
    staleTime: 1000 * 60 * 5,
    retry: false,
    networkMode: "always",
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Button 
              ...
              style={tenant ? { color: tenant.primaryColor } : undefined}
            >
        ...
        {isLoading ? (
          ...
          (Array.isArray(tab) ? tab : []).map((value) => (
            <TabsTrigger
              ...
              style={tenant && selectedTab === value ? { backgroundColor: tenant.primaryColor } : undefined}
            >
              <span className={`${
                selectedTab === value 
                  ? "text-white" 
...
const SubscriptionCard = ({ period }: { period: string }) => {
  ...
  // Define plans based on period
  const plans = useMemo(() => {
    const defaultPlans: Record<string, Array<{
      id: string;
      price: number;
      duration: string | any;
      capacity: string;
      ...
      hourly: [
        { id: "hourly-1", price: 100, duration: "1h", capacity: "1", bundle: "standard", data_bundle: 500 },
        { id: "hourly-2", price: 200, duration: "1h", capacity: "1", bundle: "premium", data_bundle: 1000 }
      ],
      daily: [
        { id: "daily-1", price: 500, duration: "1d", capacity: "1", bundle: "standard", data_bundle: 1 },
        { id: "daily-2", price: 800, duration: "1d", capacity: "1", bundle: "premium", data_bundle: 2 }
      ],
      weekly: [
        { id: "weekly-1", price: 2500, duration: "7d", capacity: "1", bundle: "standard", data_bundle: 7 },
        { id: "weekly-2", price: 3500, duration: "7d", capacity: "1", bundle: "premium", data_bundle: 14 }
      ],
      monthly: [
        { id: "monthly-1", price: 8000, duration: "30d", capacity: "1", bundle: "standard", data_bundle: 30 },
        { id: "monthly-2", price: 12000, duration: "30d", capacity: "1", bundle: "premium", data_bundle: 60 }
      ]
    };
    return defaultPlans[period] || [];
  }, [period]);  
  const handleSelectPlan = (id?: string) => {
    const findPlan = plans?.find((item) => item.id === id);
    
    if (findPlan) {
      // Use type assertion to tell TypeScript to treat this as SubscriptionCardType
      setActivePlan(findPlan as unknown as SubscriptionCardType);
      
      const [_, tab] = getPeriodLabel(findPlan.duration as any);
  ...
  return (
    <div className="w-full grid grid-cols-2 md:grid-cols-3 gap-4">
      {plans.map(({ id, data_bundle, capacity, duration, price }, idx) => {
        ...
        return (
              ...
              <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden">
                <div className={`absolute transform rotate-45 bg-opacity-90 text-white text-xs font-bold py-1 right-[-35px] top-[12px] w-[120px] text-center font-[family-name:var(--font-din-bold)]
                  ${idx % 3 === 0 ? 'bg-blue-500' : 
                    ...
                    'bg-purple-500'}`}
                    style={tenant && idx % 3 === 0 ? { backgroundColor: tenant.primaryColor } : 
                    tenant && idx % 3 === 1 ? { backgroundColor: tenant.secondaryColor || tenant.primaryColor } : 
                    undefined}>
             {tab}
           </div>
         </div>
         ...
         <Package className={`mb-1  ${
           ...
           style={tenant && idx % 3 === 0 ? { color: tenant.primaryColor } : 
                  tenant && idx % 3 === 1 ? { color: tenant.secondaryColor || tenant.primaryColor } : 
                  undefined}
         />
...
const checkAvailability = async () => {
 ...
 try {
   // If tenant is available, check tenant-specific vouchers
   if (tenant) {
     const available = await checkTenantVoucherAvailability(
       tenant.id,
       duration,
       capacity,
       data_bundle
     ...
     // Fall back to the original voucher check
   ...
   if (initialCheckRef.current && !vouchersAvailable) {
...
}, [network_location, duration, capacity, data_bundle, tenant]);
...
try {
 ...
 const voucherData = {
   ...
   bundle: data_bundle,
   network_location,
   tenantId: tenant?.id, // Include tenant ID if available
 ...
 const response = await fetch('/api/payment/initialize', {
   ...
   body: JSON.stringify({
     email: tenant ? tenant.contactEmail : (network_location === 'nana-network' ? "fromnanalodge@gmail.com" : "fromalherilodge@gmail.com"),
     amount: amount * 100,
     metadata: {
 ...
 <DrawerContent className="mx-auto sm:w-4/6 md:w-3/6 lg:w-2/6 text-center rounded-t-xl">
   ...
   <div className="space-y-4 p-6">
     <div className="bg-blue-50 rounded-xl p-4 mb-6 flex flex-col items-center"
       style={tenant ? { backgroundColor: `${tenant.primaryColor}20` } : undefined}>
       <p className="text-sm text-blue-600 mb-1 font-[family-name:var(--font-din-bold)]"
         style={tenant ? { color: tenant.primaryColor } : undefined}>Total Amount</p>
       <p className="text-3xl font-bold text-blue-700 font-[family-name:var(--font-din-bold)]"
         style={tenant ? { color: tenant.primaryColor } : undefined}>{parseToNaira(amount)}</p>
     ...
     <Button 
      ...
      style={tenant && !(isProcessing || checkingVouchers || (hasCheckedVouchers && !vouchersAvailable)) ? {
        background: `linear-gradient(to right, ${tenant.primaryColor}, ${tenant.secondaryColor || tenant.primaryColor})`
      } : undefined}
    >
      {isProcessing ? (
...
async function checkVouchersAvailability(
network_location: string,
duration: string,
capacity: string,
data_bundle?: number
...
try {
// This is a placeholder - in a real app, you would check your database
return true;
} catch (error) {
console.error("Error checking voucher availability:", error);
return false;
...
// Helper function to check tenant voucher availability
async function checkTenantVoucherAvailability(
tenantId: string,
duration: string,
capacity: string,
data_bundle?: number
...
try {
// Use the tenant-specific function to check voucher count
const count = await getTenantVoucherCount(
tenantId,
duration,
capacity,
data_bundle?.toString() || "standard"
);

return count > 0;
} catch (error) {
console.error("Error checking tenant voucher availability:", error);
return false;
}
}
